const validateDNI = require("./index.cjs")
...
// Test only DNI
if (validateDNI("X1234567L", true)) { errors.push("Failed: Only DNI(NIE should not pass)") }
if (!validateDNI("12345678Z", true)) { errors.push("Failed: Only DNI(DNI should pass)") }
...
if (errors.length > 0) {
    ...
    console.log("\x1b[32mTest succesfull(CommonJS)\x1b[0m")
}